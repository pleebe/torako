...
async.eachSeries(config.boards, (board, boardCb) => {
    async.eachSeries(require('./data/' + board + '.json'), (thread, threadCb) => {
        ...
        if (thread_data[thread]['op']['media'] !== null && thread_data[thread]['op']['media']['thumb_link'] !== null) {
            ...
            request(link, {encoding: 'binary'}, function(error, response, body) {
                console.log("Writing file " + './data/' + board + '/thumb/' + filename.substring(0, 4) + '/' + filename.substring(4, 6) + '/' + filename);
                fs.outputFile('./data/' +board + '/thumb/' + filename.substring(0, 4) + '/' + filename.substring(4, 6) + '/' + filename, body, 'binary', function (err) {});
            });
        }
        for (var i in thread_data[thread]['posts']) {
            if (thread_data[thread]['posts'][i]['media'] !== null && thread_data[thread]['posts'][i]['media']['thumb_link'] !== null) {
